...
namespace InventorySimulation
{
    public partial class Results : Form
    {
        ...
        private void Results_Load(object sender, EventArgs e)
        {
            ...
            {
                dataGridView1.Rows.Add(row.Day, row.Cycle, row.DayWithinCycle, row.BeginningInventory,row.RandomDemand,row.Demand, row.EndingInventory, row.ShortageQuantity, row.OrderQuantity, row.RandomLeadDays,row.LeadDays,row.DaysUntilOrderArrives);
            }
